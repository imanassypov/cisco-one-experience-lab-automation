...
(`dcnm_network`), and failed there after 120 seconds. Nexus Dashboard answered
HTTP 200 and put the real outcome in the per-attachment `DATA` map:

```
MainNetwork1-[93P9A1WUTSN/DC-Leaf1]-[93BDR3P5G4S/DC-Leaf2]:
  "SUCCESS Peer attach Response -  SUCCESS"
ProdNetwork1-[...]:
  "Entered network VLAN id 2300 is already in use. Peer attach Response -  Entered network VLAN id 2300 is already in use."
IOTNetwork1-[...]:
  "Entered network VLAN id 2300 is already in use. Peer attach Response -  Entered network VLAN id 2300 is already in use."
ROLLBACK_RESULT:
...
All three networks asked for VLAN 2300, the bottom of the range, so the first
attachment won and the other two were rejected. The cause is in `cisco.dcnm`
3.13.0 itself. When an object carries no VLAN, the module fills one in from the
controller - but it does so inside the loop that walks the objects it is about
to create. `plugins/modules/dcnm_vrf.py`, lines 4011-4043:
...
`GET_VLAN` is
`/appcenter/cisco/ndfc/api/v1/lan-fabric/rest/resource-manager/vlan/{fabric}?vlanUsageType=TOP_DOWN_VRF_VLAN`.
That endpoint returns the next *free* VLAN and does not reserve it. Nothing is
committed between iterations, so every VRF in a single run is handed the same
answer. `dcnm_network.py` does the same in its create loop - the GET is at 5056 and
the path constant at line 1083 - against `vlanUsageType=TOP_DOWN_NETWORK_VLAN`,
which is why all three networks asked for 2300 rather than 2300, 2301, 2302.
The lookup appears twice in `dcnm_vrf.py` - at 4030 in the update path quoted
above and again at 4469 in its create path - so there is no route through
either module that avoids it.

The general lesson is worth more than the specific bug. The guide's GUI
procedure and this data model's automation of it are not equivalent here.
Clicking "Propose VLAN" is one object at a time with a save in between, so each
proposal is committed before the next one is asked for; a batched API run asks
for all of them before committing any. Anywhere the guide relies on the
controller allocating a resource interactively, the data model has to pin what
the GUI would have proposed.

Both files are pinned now, in declaration order, from the ranges
`cisco.nac_dc_vxlan` defaults to in `roles/validate/files/defaults.yml` and
which this repo does not override - `layer3_vlan_range` 2000-2299 for VRF
VLANs and `layer2_vlan_range` 2300-2999 for network VLANs:

| File | Object | `vlan_id` |
|---|---|---|
| `playbooks/host_vars/Pseudoco-DC1/vrfs.nac.yaml` | MAIN | 2000 |
| | PROD | 2001 |
| | IOT | 2002 |
| `playbooks/host_vars/Pseudoco-DC1/networks.nac.yaml` | MainNetwork1 | 2300 |
| | ProdNetwork1 | 2301 |
| | IOTNetwork1 | 2302 |

Unlike the port-channel descriptions above, these two files are read directly
by the create role and are not regenerated by stage 00, so a plain `git pull`
is the whole fix.

**A green `vrfs` step is not evidence that the VRFs attached.** Step 15,
`vrfs` (`dcnm_vrf`), had already hit exactly this wall two steps earlier at
VLAN 2000 - and it reported `ok (changed=True)` and let the play continue. The
module does not inspect the per-attachment `DATA` map in an HTTP 200 body, so
two of the three VRF attachments were rejected by the controller in silence and
the run only stopped later, indirectly, when `dcnm_network` hit the same wall
and did fail. That is an observation about how this version of the module
reports, not a filed bug, but it changes how you debug this stage: when
something downstream of `vrfs` complains about VRFs, read the `DATA` map in the
`vrfs` response rather than trusting its `ok`.

Verified on the lab after pinning. One run completed all 21 steps -
`Pseudoco-DC1 : ok=40 changed=4 unreachable=0 failed=0 skipped=17`, with no
occurrence of "already in use" anywhere in the log - and a second run reported
