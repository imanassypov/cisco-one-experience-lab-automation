...
**`site` is the field that matters.** Catalyst Center never provisions an access point directly, and nothing lets you assign it a policy tag or a site tag. Both are derived, in two moves: the AP is assigned to a site, and then the controller managing that site is re-provisioned, at which point it takes the AP over and pushes the tags implied by the wireless profile bound to that site. An AP that has had neither done to it stays on `default-policy-tag`, which maps no WLAN, so it broadcasts nothing. This is why the block cannot simply name the AP and stop, and why stage 08 re-provisions the controller straight after assigning an AP.

The controller re-provision needs `force_provisioning`, because `provision_workflow_manager` short-circuits on *"already provisioned"* without diffing what changed underneath. Stage 08 therefore only forces it when the AP assignment actually moved something — otherwise every run would reboot the APs. If the two halves come apart, with the AP already a site member but the controller never re-provisioned afterwards, re-run with `-e force_reprovision=true` to hand it over.
