...
| `lab_ap_macs` | Leave `[]` until **after** stage 09. The composite programs the AP trunk (`Gi1/0/2`, native VLAN 10); until then the AP cannot join the WLC and Catalyst Center has no Unified AP. Wait for the AP to be Registered on the WLC **and** to appear in Catalyst Center Inventory — the sync lags the controller by a few minutes, and stage 08 resolves the AP from Catalyst Center, not from the WLC. Then put that **Ethernet** MAC here (colon-separated, first entry is `{AP1_MAC}`) and re-run stage 08, which names the AP, assigns it to Site-105, then provisions it. One join is enough. `settings.json` always carries two AP rows; an `{APn_MAC}` with no list entry is skipped. Do not use the Base Radio MAC. |
